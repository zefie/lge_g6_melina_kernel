...
*********************** remove the code for TZ5.0 by mono *********************************/
#if 0
	if((rc = of_property_read_u32(gf_dev->spi->dev.of_node, "qcom,qup-id", &gf_dev->qup_id)) < 0)
	{
		dev_err(&gf_dev->spi->dev,"Error getting qup_id\n");
		return -GF_PERM_ERROR;
	}
#endif
	if((rc = of_property_read_u32(gf_dev->spi->dev.of_node, "gf,vddio-uV", &gf_dev->vddio_uV)) < 0)
	{
        ...
        gf_dbg("gf: hw_rev: %d",gf_dev->vddio_uV);
