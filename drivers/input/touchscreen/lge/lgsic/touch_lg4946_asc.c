...
{
	struct lg4946_data *d = to_lg4946_data(dev);
	struct lg4946_asc_info *asc = &(d->asc);

	if (!lg4946_asc_usable(dev))
		return false;
...
static int lg4946_asc(struct device *dev, u32 code, u32 value)
{
	struct lg4946_data *d = to_lg4946_data(dev);
	u32 rdata = 0;
	u32 wdata = 0;
	...
	case ASC_READ_MAX_DELTA:
		lg4946_reg_read(dev, MAX_DELTA, &rdata, sizeof(rdata));
	...
	case ASC_GET_FW_SENSITIVITY:
		lg4946_reg_read(dev, TOUCH_MAX_R, &rdata, sizeof(rdata));
	...
	case ASC_WRITE_SENSITIVITY:
		...
		lg4946_reg_write(dev, TOUCH_MAX_W, &wdata, sizeof(wdata));
	...
	case ASC_WRITE_ONHAND:
		if ((value == NOT_IN_HAND) || (value == IN_HAND)) {
			lg4946_reg_read(dev, ONHAND_R, &rdata, sizeof(rdata));
			wdata = value;
			if (rdata == wdata)
				break;
			lg4946_reg_write(dev, ONHAND_W, &wdata, sizeof(wdata));
...
static void lg4946_asc_get_delta(struct device *dev)
{
	...
	if (!lg4946_asc_delta_chk_usable(dev)) {
		TOUCH_I("%s : lg4946_asc_delta_chk_usable() error\n", __func__);
		return;
	}
	...
	mutex_lock(&ts->lock);
	delta = lg4946_asc(dev, ASC_READ_MAX_DELTA, 0);
...
static void lg4946_asc_change_sensitivity(struct device *dev,
		int next_sensitivity)
{
	...
	if (!lg4946_asc_usable(dev)) {
		TOUCH_I("%s : lg4946_asc_usable() error\n", __func__);
		return;
	}
	...
	lg4946_asc(dev, ASC_WRITE_SENSITIVITY, next_sensitivity);
...
static void lg4946_asc_update_sensitivity(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct lg4946_data *d = to_lg4946_data(dev);
	struct lg4946_asc_info *asc = &(d->asc);
	...
	mutex_lock(&ts->lock);
	lg4946_asc_change_sensitivity(dev, next_sensitivity);
	mutex_unlock(&ts->lock);

	return;
}

static void lg4946_asc_finger_input_check_work_func(
		struct work_struct *finger_input_work)
{
	struct lg4946_asc_info *asc =
		container_of(to_delayed_work(finger_input_work),
				struct lg4946_asc_info, finger_input_work);
	struct lg4946_data *d = asc->d;
	struct touch_core_data *ts = to_touch_core(d->dev);

	if (!lg4946_asc_delta_chk_usable(d->dev)) {
		TOUCH_I("%s : lg4946_asc_delta_chk_usable() error\n", __func__);
		return;
	}

	if ((ts->tcount == 1) && !(asc->delta_updated))
		lg4946_asc_get_delta(d->dev);

	if ((ts->tcount == 0) && asc->delta_updated) {
		if (asc->curr_delta_cnt < asc->max_delta_cnt)
			asc->delta_updated = false;
		else
			lg4946_asc_update_sensitivity(d->dev);
	}
...
void lg4946_asc_toggle_delta_check(struct device *dev)
{
	...
	if ((connect_status != CONNECT_INVALID) ||
		...
		case NOT_IN_HAND:
			...
			lg4946_asc_change_sensitivity(dev, ACUTE_SENSITIVITY);
			break;
		case IN_HAND:
			asc->use_delta_chk = DELTA_CHK_ON;
			lg4946_asc_change_sensitivity(dev, NORMAL_SENSITIVITY);
...
void lg4946_asc_update_qcover_status(struct device *dev, int new_qcover)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void lg4946_asc_init_works(struct device *dev)
{
	struct lg4946_data *d = to_lg4946_data(dev);

	INIT_DELAYED_WORK(&(d->asc.finger_input_work),
			lg4946_asc_finger_input_check_work_func);
...
{
	lg4946_asc(dev, ASC_WRITE_ONHAND, value);
...
void lg4946_asc_control(struct device *dev, bool on_off)
{
	...
	if (on_off == ASC_ON) {
		lg4946_asc(dev, ASC_GET_FW_SENSITIVITY, 0);
		...
		asc->use_asc = ASC_OFF;
		touch_msleep(50);
		lg4946_asc(dev, ASC_WRITE_SENSITIVITY, NORMAL_SENSITIVITY);
...
static ssize_t show_lg4946_asc_param(struct device *dev, char *buf)
{
	struct lg4946_data *d = to_lg4946_data(dev);
	struct lg4946_asc_info *asc = &(d->asc);
	int ret = 0;
...
static ssize_t store_lg4946_asc_param(struct device *dev, const char *buf,
		size_t count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct lg4946_data *d = to_lg4946_data(dev);
...
static ssize_t show_lg4946_asc_onhand(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int value = 0;
	int ret = 0;
...
static ssize_t store_lg4946_asc_onhand(struct device *dev, const char *buf,
		size_t count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int value = 0;
...
static TOUCH_ATTR(asc, show_lg4946_asc_param, store_lg4946_asc_param);
static TOUCH_ATTR(onhand, show_lg4946_asc_onhand, store_lg4946_asc_onhand);

static struct attribute *lg4946_asc_attribute_list[] = {
	&touch_attr_asc.attr,
	&touch_attr_onhand.attr,
	NULL,
};

static const struct attribute_group lg4946_asc_attribute_group = {
	.attrs = lg4946_asc_attribute_list,
};
...
void lg4946_asc_register_sysfs(struct device *dev)
{
	...
	ret = sysfs_create_group(&ts->kobj, &lg4946_asc_attribute_group);
...
static void lg4946_asc_get_dts(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct lg4946_data *d = to_lg4946_data(dev);
...
void lg4946_asc_init(struct device *dev)
{
	...
	memset(&(d->asc), 0, sizeof(struct lg4946_asc_info));

	d->asc.d = d;

	lg4946_asc_init_works(dev);
	lg4946_asc_get_dts(dev);
