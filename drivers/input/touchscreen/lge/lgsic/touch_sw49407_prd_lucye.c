...
#include "touch_sw49407_prd_lucye.h"
...
static int write_test_mode(struct device *dev, u32 type)
{
	...
	/* TestType Set */
	sw49407_reg_write(dev, d->reg_info.r_tc_cmd_spi_addr + tc_tsp_test_ctl,
			(u8 *)&testmode,
			sizeof(testmode));
	...
	do {
		touch_msleep(100);
		sw49407_reg_read(dev, d->reg_info.r_tc_sts_spi_addr + 42,
				(u8 *)&rdata,
				sizeof(rdata));
		TOUCH_I("rdata = 0x%x\n", rdata);
	...
	u16 (*buf)[COL_SIZE], int type)
{
	...
	sw49407_write_value(dev, spr_data_offset,
		d->reg_info.r_sys_buf_sram_oft + PT_FRAME_SIZE * 3 / 4);

	sw49407_reg_read(dev, data_access_addr,
...
static int read_pt_frame_1(struct device *dev, struct sw49407_data *d, u32 type)
{
	u32 frame_size = 0;
	...
	if (type == U3_OPEN_NODE_TEST || type == U3_M2_RAWDATA_TEST ||
		type == U2_M2_RAWDATA_TEST || type == U0_M2_RAWDATA_TEST) {
		x_size = 18;
		y_size = 34;
	...
	} else if (type == U0_M1_RAWDATA_TEST) {
		x_size = 2;
		y_size = 34;
		is_flip_x_y_data = 1;
		buf_ptr = M1_Rawdata_buf;
	...
	if (frame_size > 0) {
		if (is_flip_x_y_data) {
			sw49407_write_value(dev, spr_data_offset,
						d->reg_info.r_sys_buf_sram_oft);
			sw49407_reg_read(dev, data_access_addr, (u32*)tmpbuf,
						frame_size * sizeof(u16));

			memset(buf_ptr, 0x0, frame_size);

			for(x = 0; x < x_size; x++) {
				for(y = 0; y < y_size; y++) {
					buf_ptr[y * 18 + x] = tmpbuf[cur];
					cur++;
				}
			}
		} else {
			sw49407_write_value(dev, spr_data_offset,
						d->reg_info.r_sys_buf_sram_oft);
			sw49407_reg_read(dev, data_access_addr, (u32*)buf_ptr,
						frame_size * sizeof(u16));
		}
	}
...
static int read_pt_frame_2(struct device *dev, struct sw49407_data *d, u32 type)
{
	u32 frame_size = 0;
	int is_flip_x_y_data = 0;
	int x_size = 0;
	...
	if (type == U3_OPEN_NODE_TEST || type == U3_M2_NOISE_TEST ||
		type == U3_M2_DELTA_JITTER || type == U0_M2_NOISE_TEST) {
		x_size = 18;
		y_size = 34;
		buf_ptr = M2_Rawdata_buf;
		memset((u16*)buf_ptr, 0x0, ROW_SIZE*COL_SIZE*sizeof(u16));
		...
		x_size = 0;
		y_size = 0;
		memset((u8*)W_Buf, 0x0, PT_FRAME_SIZE);
		return 1;
	}
	...
	if (frame_size > 0) {
		if (is_flip_x_y_data) {
			sw49407_write_value(dev, spr_data_offset,
			d->reg_info.r_sys_buf_sram_oft + PT_FRAME_SIZE * 1 / 4);
			sw49407_reg_read(dev, data_access_addr, (u32*)tmpbuf,\
				frame_size * sizeof(u16));
...
static int read_pt_frame_3(struct device *dev, struct sw49407_data *d, u32 type)
{
	u32 frame_size = 0;
	u16* buf_ptr;
	int x_size = 0;
	...
	if (type == U3_OPEN_NODE_TEST || type == U3_M2_DELTA_JITTER) {
		x_size = 18;
		y_size = 34;
		buf_ptr = M2_Rawdata_buf;
		memset((u16*)buf_ptr, 0x0, ROW_SIZE*COL_SIZE*sizeof(u16));
	...
	if (frame_size > 0) {
		sw49407_write_value(dev, spr_data_offset,
			d->reg_info.r_sys_buf_sram_oft + PT_FRAME_SIZE * 2 / 4);
		sw49407_reg_read(dev, data_access_addr,
				(u32*)buf_ptr, frame_size * sizeof(u16));
	}

	return frame_size;
}

static int read_pt_frame_4(struct device *dev, struct sw49407_data *d, u32 type)
{
	u32 frame_size = 0;
	int x_size = 0;
	int y_size = 0;
	...
	if (type == U3_OPEN_NODE_TEST || type == U3_M2_NOISE_TEST ||
		type == U0_M2_RAWDATA_TEST || type == U0_M2_NOISE_TEST) {
		x_size = 18;
		y_size = 34;
		buf_ptr = M2_Rawdata_buf;
		memset((u16*)buf_ptr, 0x0, ROW_SIZE*COL_SIZE*sizeof(u16));
...
static int prd_open_short_test(struct device *dev)
{
	...
	sw49407_reg_read(dev, d->reg_info.r_tc_sts_spi_addr + 43,
...
static int sdcard_spec_file_read(struct device *dev)
{
	int ret = 0;
	int fd = 0;
	char *path[2] = { "/mnt/sdcard/elsa_limit.txt",
		"/mnt/sdcard/elas_limit_mfts.txt"
	};
	int path_idx = 0;
...
static int prd_compare_rawdata(struct device *dev, u32 type)
{
	...
	for (i = 0; i < ROW_SIZE; i++) {
		for (j = 0; j < col_size; j++) {
			if ((rawdata_buf[i*18+j] < LowerImage[i][j]) ||
					(rawdata_buf[i*18+j] >
					UpperImage[i][j])) {
				if ((type != U0_M1_RAWDATA_TEST) &&
						(i <= 1 && j <= 4)) {
...
static void tune_display(struct device *dev, struct sw49407_tune_data *t,
								int type)
{
...
//LOFT
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m1_left, log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m1_right, log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
	...
	case U0_M2_RAWDATA_TEST:
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m2_g1_left,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m2_g1_right,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m2_g2_left,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m2_g2_right,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m2_g3_left,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u0_m2_g3_right,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
	...
	case U3_M2_RAWDATA_TEST:
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u3_m2_g1_left,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u3_m2_g1_right,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u3_m2_g2_left,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u3_m2_g2_right,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u3_m2_g3_left,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
		...
		write_file(dev, log_buf, TIME_INFO_SKIP);
		ret = 0;
		ret += tune_loft_print(17, t->r_loft_tune_u3_m2_g3_right,
								log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
		write_file(dev, log_buf, TIME_INFO_SKIP);
...
{
	struct sw49407_data *d = to_sw49407_data(dev);
	struct sw49407_tune_data t;

	sw49407_reg_read(dev, d->reg_info.r_tune_code_spi_addr, &t,
				sizeof(struct sw49407_tune_data));
	write_file(dev, "\n[Read Tune Code]\n", TIME_INFO_SKIP);
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	pt_command = 0x300;
	sw49407_reg_write(dev, 0xC04, &pt_command, sizeof(int));
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	do {
		...
		if (retry != 1000)
			touch_msleep(5);
		sw49407_reg_read(dev, d->reg_info.r_abt_sts_spi_addr +
	...
	sw49407_reg_write(dev, spr_data_offset, (u8 *)&data_offset,
								sizeof(u32));

	sw49407_reg_read(dev, data_access_addr, (u8 *)buf, __frame_size);
