...
static int msm_ispif_restart_frame_boundary(struct ispif_device *ispif,
	struct msm_ispif_param_data *params)
{
//LGE_UPDATE_S restart ispif for dualFOV(CN#02793393) 20170202 yt.jeon@lge.com
#if 0
	int rc = 0;
	...
	pr_err("%s: ISPIF reset hw done, Restarting", __func__);
	rc = msm_cam_clk_enable(&ispif->pdev->dev,
		ispif->clk_info, ispif->clks,
		ispif->num_clk, 0);
	if (rc < 0)
...
//LGE_UPDATE_E restart ispif for dualFOV(CN#02793393) 20170202 yt.jeon@lge.com
	return rc;
}
