...
//BT_S : [CONBT-5284] Change lock API to avoid kernel crash
struct mutex mutex_store_install;
//BT_E : [CONBT-5284] Change lock API to avoid kernel crash
...
//BT_S : [CONBT-2025] Fix snoop log issue 30(used) => 28
...
//    spin_lock_irqsave(&hu->err_lock, flags);
    mutex_lock(&mutex_store_install);
//BT_E : [CONBT-5284] Change lock API to avoid kernel crash
...
//    spin_unlock_irqrestore(&hu->err_lock, flags);
    mutex_unlock(&mutex_store_install);
//BT_E : [CONBT-5284] Change lock API to avoid kernel crash
    return size;
}
...
//BT_S : [CONBT-4509][CSP#1128901] Cleanup completion timer after uart close
        if(mutex_is_locked(&cmd_credit))
        {
            complete_all(&hu->cmd_rcvd);
        }
...
//BT_S : Build error in the new v4l2 driver
//        INIT_COMPLETION(hu->ldisc_installed);
    reinit_completion (&hu->ldisc_installed);
//BT_E : Build error in the new v4l2 driver
...
//BT_S : [CONBT-5284] Change lock API to avoid kernel crash
    mutex_init(&mutex_store_install);
//BT_E : [CONBT-5284] Change lock API to avoid kernel crash
...
//BT_S : [CONBT-5284] Change lock API to avoid kernel crash
    mutex_destroy(&mutex_store_install);
//BT_E : [CONBT-5284] Change lock API to avoid kernel crash
