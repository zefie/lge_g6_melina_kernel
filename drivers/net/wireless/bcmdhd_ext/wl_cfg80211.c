 ...
 *
 * $Id: wl_cfg80211.c 684100 2017-02-10 08:45:02Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
#define BTM_QUERY_IDX_DIALOG_TOKEN	2
#define BTM_QUERY_REASON			6	/* better AP found */

typedef struct wl_btm_bssid {
	struct ether_addr ea;
	struct list_head list;
} wl_btm_bssid_t;

static void wl_cfg80211_send_btm_query(struct bcm_cfg80211 *cfg, struct net_device *dev);
static bool wl_cfg80211_add_btm_bssid_list(struct bcm_cfg80211 *cfg, struct ether_addr *ea);
static void wl_cfg80211_clear_btm_bssid_list(struct bcm_cfg80211 *cfg);
#endif /* WBTEXT */
...
#ifdef CONFIG_MELINA_BCMDHD_MONITOR_MODE
...
#if defined(CUSTOMER_HW10) && defined(WLTDLS)
			wl_tdls_enable(cfg);
#endif
...
#endif
#ifdef SUPPORT_SET_CAC
			WL_DBG(("P2P Connect, set CAC 0\n"));
			if (wl_cfg80211_set_cac(cfg, 0)) {
				WL_ERR(("Failed Set CAC\n"));
			}
#endif	/* SUPPORT_SET_CAC */
...
#ifdef SUPPORT_SET_CAC
			WL_DBG(("P2P disconnect, set CAC 1\n"));
			if (wl_cfg80211_set_cac(cfg, 1)) {
				WL_ERR(("Failed Set CAC\n"));
			}
#endif	/* SUPPORT_SET_CAC */
		}
	...
	struct vif_params *params)
{
	...
	s32 wlif_type;
	s32 mode = 0;
#ifndef CONFIG_MELINA_BCMDHD_MONITOR_MODE
	s32 err = BCME_OK;
#endif
	s32 index;
	s32 conn_idx = -1;
	...
	if (!ap) {
		wldev_ioctl_set(ndev, WLC_SET_INFRA, &infra, sizeof(s32));
		wldev_ioctl_set(ndev, WLC_SET_PROMISC, &promisc, sizeof(s32));
		wldev_ioctl_set(ndev, WLC_SET_MONITOR, &mon, sizeof(s32));
		wl_set_mode_by_netdev(cfg, ndev, mode);
	}
#else
	if (ibss) {
	...
	struct cfg80211_ssid *this_ssid)
{
	...
	if (cfg->scansuppress_mode) {
		WL_ERR(("priv-command scansuppress enabled\n"));
		return -EPERM;
	}
#endif
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	WL_TRACE_HW4(("%s : caller process (%s)\n", __FUNCTION__, current->comm)); //LGE_Patch
	...
	u8 *mac_addr, const char *name)
{
	...
	ret = wl_alloc_netinfo(cfg, new_ndev, wdev,
...
#if defined(CONFIG_TCPACK_FASTTX)
	...
	struct cfg80211_connect_params *sme)
{
...
#elif defined(SUPPORT_AP_POWERSAVE)
	dhd_pub_t *dhd =  (dhd_pub_t *)(cfg->pub);
	...
	wl_cfg80211_set_random_mac(FALSE);
#endif /* SUPPORT_RANDOM_MAC_SCAN */

#if defined(SUPPORT_AP_POWERSAVE)
	WL_INFORM(("disable ap powersave\n"));
	dhd_set_ap_powersave(dhd, 0, FALSE);
#endif /* SUPPORT_AP_POWERSAVE */
...
#endif
	bzero(&bssid, sizeof(bssid));
	if ((ret = wldev_ioctl_get(dev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN)) == 0) {
	...
	*/
	...
	/* CSP#1001232, Ralink AP has EAP_TLS issue due to runtime PM, so disable RuntimePM from
	 * join request to wl_notify_connect_status has done
	 * Now will try to join to the AP. Disable RuntimePM until connect has done or faild
	 */
#if (defined(CUSTOMER_HW10) && defined(DHD_PCIE_RUNTIMEPM))
	DHD_DISABLE_RUNTIME_PM((dhd_pub_t *)cfg->pub);
#endif /* CUSTOMER_HW10 && DHD_PCIE_RUNTIMEPM */
	...
	u16 reason_code)
{
	...
	if (act) {
		...
		* Cancel ongoing scan to sync up with sme state machine of cfg80211.
		*/
#if (defined(BCM4334_CHIP) || !defined(ESCAN_RESULT_PATCH) || defined(CUSTOMER_HW10))
...
#if defined(PRIVATE_SCANSUPPRESS)
void wl_cfg80211_set_scan_suppress(struct net_device *dev, char *command)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	char mode = 0;

	mode = command[strlen("SET_SCANSUPPRESS") + 1] - '0';

	if (mode == 1)
		cfg->scansuppress_mode = 1;
	else if(!mode)
		cfg->scansuppress_mode = 0;
	else
		WL_ERR(("Unknown command \n"));

	WL_ERR(("%s scansuppress_mode: %d\n",__FUNCTION__, mode));
}

int wl_cfg80211_get_scan_suppress(struct net_device *dev, char *command)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	return cfg->scansuppress_mode;
}
...
#ifdef CUSTOMER_HW10
		set_parallelscan(dhdp);
#endif /* CUSTOMER_HW10 */


#if defined(CUSTOMER_HW10) && defined(SOFTAP_TPUT_ENHANCE)
        set_softap_params(dhdp);
#endif
...
#ifdef WL_SCHED_SCAN
//LGE_Patch
#define PNO_TIME		120 //120 // 30
#define PNO_REPEAT		2 //2 // 4
...
#if (!defined(WL_ENABLE_P2P_IF) && !defined(WL_CFG80211_P2P_DEV_IF)) || defined(CONFIG_MELINA_BCMDHD_MONITOR_MODE)
	...
	const wl_event_msg_t *e, void *data)
{
	...
	u8 *bssid = 0;
#if (defined(WBTEXT) || defined(SUPPORT_AP_POWERSAVE))
	dhd_pub_t *dhdp = (dhd_pub_t *)cfg->pub;
#endif /* WBTEXT || SUPPORT_AP_POWERSAVE */
		...
		if (wl_is_linkup(cfg, e, ndev)) {
...
#ifdef SUPPORT_AP_POWERSAVE
			if (dhdp->op_mode & (DHD_FLAG_HOSTAP_MODE | DHD_FLAG_P2P_GO_MODE)) {
				WL_INFORM(("enable radio power save \n"));
				dhd_set_ap_powersave(dhdp, 0, TRUE);
			}
#endif /* SUPPORT_AP_POWERSAVE */
			...
			if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
				...
				if (ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION &&
					...
					wl_cfg80211_clear_btm_bssid_list(cfg);
				}
#endif /* WBTEXT */
			}
	...
	const wl_event_msg_t *e, void *data)
{
	 ...
	 * roamed bssid are same and the channels are same
	 * Also clear timer roam_timeout.
	 */
	curbssid = wl_read_prof(cfg, ndev, WL_PROF_BSSID);
	channel = (u32 *)wl_read_prof(cfg, ndev, WL_PROF_CHAN);
	if ((wldev_ioctl_get(ndev, WLC_GET_CHANNEL, &ci, sizeof(ci))) < 0) {
	...
	const wl_event_msg_t *e, void *data, bool completed)
{
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
		...
		if (completed && (wl_cfg80211_verify_bss(cfg, ndev) != true)) {
			 ...
			 */
			WL_ERR(("BSS entry not found. Avoid assoc event indication\n"));
...
#ifdef WBTEXT
	INIT_LIST_HEAD(&cfg->btm_bssid_list);
#endif /* WBTEXT */
...
#ifdef CONFIG_MELINA_BCMDHD_MONITOR_MODE
	err = wldev_ioctl_set(ndev, WLC_SET_PROMISC, &promisc, sizeof(s32));
	if (unlikely(err)) {
		WL_ERR(("WLC_SET_PROMISC error (%d)\n", err));
		return err;
	}
	err = wldev_ioctl_set(ndev, WLC_SET_MONITOR, &mon, sizeof(mon));
	if (unlikely(err)) {
...
#ifdef SUPPORT_SET_CAC
		WL_DBG(("TDLS Disconnect, set CAC 1\n"));
		if (wl_cfg80211_set_cac(cfg, 1)) {
			WL_ERR(("Failed Set CAC\n"));
		}
#endif	/* SUPPORT_SET_CAC */
...

{
	...
	if (CHSPEC_IS5G(chanspec))
		freq = ieee80211_channel_to_frequency(channel, (enum ieee80211_band)NL80211_BAND_5GHZ);
	else
		freq = ieee80211_channel_to_frequency(channel, (enum ieee80211_band)NL80211_BAND_2GHZ);
...
wl_cfg80211_set_spect(struct net_device *dev, int spect)
{
	...
	if (!wl_get_drv_status_all(cfg, CONNECTED)) {
		err = wldev_ioctl_set(dev, WLC_DOWN, &down, sizeof(down));
		if (err) {
		...
		err = wldev_ioctl_set(dev, WLC_UP, &up, sizeof(up));
		if (err) {
...
#if defined(SUPPORT_RANDOM_MAC_SCAN)
int
wl_cfg80211_set_random_mac(bool enable)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	int ret;
	...
	if (enable) {
		ret = wl_cfg80211_random_mac_enable();
	} else {
		ret = wl_cfg80211_random_mac_disable();
	}
...
int
wl_cfg80211_random_mac_enable(void)
{
...
int
wl_cfg80211_random_mac_disable(void)
{
	s32 err = BCME_ERROR;
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
...
wl_cfg80211_set_cac(struct bcm_cfg80211 *cfg, int enable)
{
	...
	WL_DBG(("enable %d, op_mode 0x%04x\n", enable, dhd->op_mode));
	if (!dhd) {
		WL_ERR(("dhd is NULL\n"));
		return -ENODEV;
	}
...
#pragma GCC diagnostic ignored "-Wcast-qual"
	...
	/* check existence */
	list_for_each_entry(bssid, &cfg->btm_bssid_list, list) {
		if (!memcmp(bssid->ea.octet, ea, ETHER_ADDR_LEN)) {
			WL_ERR(("%s: already exist\n", __FUNCTION__));
			return FALSE;
		}
	}

	bssid = kmalloc(sizeof(wl_btm_bssid_t), GFP_KERNEL);
	if (bssid == NULL) {
		WL_ERR(("%s: alloc failed\n", __FUNCTION__));
		return FALSE;
	}
	...
	list_add_tail(&bssid->list, &cfg->btm_bssid_list);

	WL_DBG(("add BTM bssid : %s\n", bcm_ether_ntoa(ea, eabuf)));
...
static void wl_cfg80211_clear_btm_bssid_list(struct bcm_cfg80211 *cfg)
{
	...
	while (!list_empty(&cfg->btm_bssid_list)) {
		bssid = list_entry(cfg->btm_bssid_list.next, wl_btm_bssid_t, list);
		if (bssid) {
			WL_DBG(("clear BTM bssid : %s\n", bcm_ether_ntoa(&bssid->ea, eabuf)));
			list_del(&bssid->list);
			kfree(bssid);
		}
	}
...
static void wl_cfg80211_send_btm_query(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	...
	int tmp_channel = 0;
	struct channel_info ci;
	uint8 btm_query[] = {DOT11_ACTION_CAT_WNM, DOT11_WNM_ACTION_BSSTRANS_QUERY, 0,
		BTM_QUERY_REASON};
	struct wl_connect_info *conn_info = wl_to_conn(cfg);
	bool btm_support = FALSE;
	bcm_tlv_t * ext_cap_ie = NULL;

	WL_DBG(("Enter\n"));

	/* check BTM bit in extend cap. IE of assoc response */
	if ((ext_cap_ie = bcm_parse_tlvs(conn_info->resp_ie, conn_info->resp_ie_len,
	...
	if (btm_support == FALSE) {
		WL_DBG(("AP doesn't support BTM\n"));
		return;
	}
	...
	if (wl_cfg80211_add_btm_bssid_list(cfg, &tmp_bssid) == FALSE) {
		WL_DBG(("BTM query cancelled\n"));
		goto send_btm_query_out;
	}

	btm_query[BTM_QUERY_IDX_DIALOG_TOKEN] = cfg->btm_query_dialog_token++;
	...
	action_frame->len = ARRAYSIZE(btm_query);
	memcpy(action_frame->data, btm_query, action_frame->len);
...
#ifdef CUSTOMER_HW10
int wl_cfg80211_need_restrict_vsdb(struct net_device *dev)
{
	struct net_info *iter, *next;
	struct net_device *p2p_ndev = NULL;
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	char *ifname = "p2p-wlan0-0";
	u32 chspec;
	s32 txchain;
	int err;
	...
	/* Check MCC MODE */
	if (!cfg->vsdb_mode) {
		WL_ERR(("%s: Not in MCC mode\n", __FUNCTION__));
		return FALSE;
	}

	/* Check Txchain */
	err = wldev_iovar_getint(dev, "txchain", (s32 *)&txchain);
	if (unlikely(err)) {
		WL_ERR(("%s: Could not get txchain %d\n", __FUNCTION__,  err));
		return FALSE;
	}
	if (txchain == 1) {
		WL_ERR(("%s: txchain is already %d\n", __FUNCTION__, txchain));
		return FALSE;
	}
	...
	for_each_ndev(cfg, iter, next) {
		if (iter->ndev) {
			if (strcmp(iter->ndev->name, ifname) == 0) {
				break;
			}
		}
	...
	if (unlikely(err)) {
		WL_ERR(("%s: Get chanspec error: %d \n", __FUNCTION__, err));
		return FALSE;
	}

	if ((chspec |= (WL_CHANSPEC_BAND_5G)) && (CHSPEC_IS20(chspec))) {
		WL_INFORM(("%s: 5GHz. BW20. VSDB. need to be restricted!\n", __FUNCTION__));
		return TRUE;
	}
	return FALSE;
}
