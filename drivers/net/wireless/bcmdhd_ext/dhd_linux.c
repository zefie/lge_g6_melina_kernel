 ...
 *
 * $Id: dhd_linux.c 680699 2017-01-23 00:58:03Z $
 */
...
#if (defined(DHD_TCP_WINSIZE_ADJUST) && !defined(CUSTOMER_HW10))
#include <linux/tcp.h>
#include <net/tcp.h>
#endif /* defined(DHD_TCP_WINSIZE_ADJUST) && !defined(CUSTOMER_HW10) */
...
extern int dhd_rsdb_mode_from_file(dhd_pub_t *dhd);
#endif
...
#if defined(CUSTOMER_HW10) && defined(OTP_WRITE_ON)
extern int dhd_write_otp(dhd_pub_t *dhd);
#endif /* CUSTOMER_HW10 && OTP_WRITE_ON */
...
static void dhd_ap_powersave_handler(void *handle, void *event_info, u8 event);
#endif
...
/* kernel pm wakelock ms on receiving rx packet */
uint dhd_rxframe_wakelock_ms = DHD_PACKET_TIMEOUT_MS;
module_param(dhd_rxframe_wakelock_ms, uint, 0644);
...
static char *logstrs_path = "/data/logstrs.bin";
static char *st_str_file_path = "/data/rtecdc.bin";
static char *map_file_path = "/data/rtecdc.map";
static char *rom_st_str_file_path = "/data/roml.bin";
static char *rom_map_file_path = "/data/roml.map";
#else /* CUSTOMER_HW4_DEBUG || CUSTOMER_HW2 */
...
static void dhd_init_logstrs_array(dhd_event_log_t *temp);
static int dhd_read_map(char *fname, uint32 *ramstart, uint32 *rodata_start, uint32 *rodata_end);
static void dhd_init_static_strs_array(dhd_event_log_t *temp, char *str_file, char *map_file);
#endif /* SHOW_LOGTRACE */
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
...
#if defined(DHD_USE_EARLYSUSPEND) || defined(CUSTOMER_HW10)
	...
	uint nd_ra_filter = 0;
	int ret = 0;
#endif /* DHD_USE_EARLYSUSPEND || CUSTOMER_HW10 */
...
#endif /* CONFIG_CONTROL_PM || CONFIG_PM_LOCK */
				dhd_iovar(dhd, 0, "roam_off", (char *)&roamvar, sizeof(roamvar),
						NULL, 0, TRUE);
#endif /* ENABLE_FW_ROAM_SUSPEND */
#ifdef NDO_CONFIG_SUPPORT
...
dhd_set_mcast_list_handler(void *handle, void *event_info, u8 event)
{
	...
	if (ifp == NULL || ifp->idx >= DHD_MAX_IFS || !dhd->pub.up) {
		DHD_ERROR(("%s: interface info not available/down \n", __FUNCTION__));
		goto done;
	}
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (ntoh16(skb->protocol) == ETHER_TYPE_BRCM) {
			...
			if (!tout_ctrl)
				tout_ctrl = dhd_rxframe_wakelock_ms;
#ifdef WLBTAMP
			...
			if (event_type == WLC_E_PFN_NET_FOUND) {
				...
				tout_ctrl = CUSTOM_PNO_EVENT_LOCK_xTIME * dhd_rxframe_wakelock_ms;
			}
#endif /* PNO_SUPPORT */
			...
			tout_rx = dhd_rxframe_wakelock_ms;
...
#ifdef CUSTOMER_HW10
		dhd_adjust_tcp_winsize(ifidx, ntoh16(skb->protocol), dhdp->op_mode, skb);
#else
...
#if defined(BCMPCIE) && defined(DHDTCPACK_SUPPRESS)
#if defined(SET_RPS_CPUS) || defined(SET_TCPACK_SUPPRESS)
		dhd_tcpack_suppress_set(&dhd->pub, TCPACK_SUP_OFF);
#else
...
dhd_init_logstrs_array(dhd_event_log_t *temp)
{
	...
	/* Make sure previous allocated memory should not exist */
...
#if defined(OTP_WRITE_ON) && defined(CUSTOMER_HW10)
	if ((!op_mode && dhd_get_fw_mode(dhd) == DHD_FLAG_MFG_MODE) ||
		(op_mode == DHD_FLAG_MFG_MODE)) {
		dhd_write_otp(dhdp);
	}
#endif /* OTP_WRITE_ON && CUSTOMER_HW10 */
	DHD_PERIM_UNLOCK(dhdp);
	return 0;
}
#ifdef WLTDLS
...
#define RXCHAIN_PWRSAVE_DISABLE			0
...
static void dhd_ap_powersave_handler(void *handle, void *event_info, u8 event)
{
	...
	if (event != DHD_WQ_WORK_AP_POWER_SAVE)
		DHD_ERROR(("%s: unexpected event \n", __FUNCTION__));

	dhd_set_ap_powersave(&dhd->pub, 0, !dhd->pwrsave_force_disable &&
		dhd->pub.pwrsave_enable);
}

#define SDB_ENABLE_AP  0x01
#define SDB_IS_AP(i)   (i & SDB_ENABLE_AP)

int dhd_ap_powersave_sdb_transition(dhd_pub_t *dhdp, void* event_data)
{
	...
	if (!dhdp->pwrsave_enable) {
		DHD_INFO(("ap power save not enabled\n"));
		return BCME_OK;
	}
	...
	dhdp->info->pwrsave_force_disable = TRUE;

	if (rdata->enable_bsscfg == 1) {
		value = &rdata->values[0];
		if (SDB_IS_AP(value->is_iftype))
		dhdp->info->pwrsave_force_disable = FALSE;
	}

	dhd_deferred_schedule_work(dhdp->info->dhd_deferred_wq, NULL,
		DHD_WQ_WORK_AP_POWER_SAVE,
		dhd_ap_powersave_handler, DHD_WORK_PRIORITY_LOW);
	return BCME_OK;
}
	...
	if (!strcmp(name, "country")) {
		...
		if (revstr) {
			...
			get_customized_country_code(dhd->info->adapter,
				(char *)&cspec.country_abbrev, &cspec, dhd->info->pub.dhd_cflags);
#else
			get_customized_country_code(dhd->info->adapter,
		...
		DHD_ERROR(("config country code[%s], table [%s], rev[%d] !!\n",
			value, cspec.country_abbrev, cspec.rev));
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if ((!op_mode && dhd_get_fw_mode(dhd->info) == DHD_FLAG_HOSTAP_MODE) ||
		...
		dhd_set_ap_powersave(dhd, 0, RXCHAIN_PWRSAVE_ENAB_ALL);
#else
...
static void async_dhd_module_init(void *data, async_cookie_t cookie)
{
	int err;
...
dhd_module_init(void){
	async_schedule(async_dhd_module_init, NULL);
	return 0;
}
...
device_initcall_sync(dhd_module_init);
#else
             ...
             wlc_roam_exp_params_t *roam_param)
{
	...
	err = dhd_iovar(&(dhd->pub), 0, "roam_exp_params",
			(char *)&roam_exp_cfg, sizeof(roam_exp_cfg), NULL, 0, TRUE);
	if (err < 0) {
	...
	uint32 mode, uint32 enable)
{
	...
	DHD_ERROR(("Enable APF filter, id=%d, ret=%d\n", filter_id, ret));
exit:
	if (buf) {
		kfree(buf);
	}
		...
		if (!dhdp->hang_was_sent) {
			...
			if (!dhdp->info->duart_execute)
				dhd_deferred_schedule_work(dhdp->info->dhd_deferred_wq,
					...
					dhd_debug_uart_exec_rd, DHD_WORK_PRIORITY_HIGH);
...
#endif /* DHD_TCP_WINSIZE_ADJUST && !CUSTOMER_HW10 */
...
do_dhd_log_dump(dhd_pub_t *dhdp)
{
	...
	common_info_len =
		strlen(pre_strs) + strlen(dhd_version) + strlen(fw_version);
	common_info = vmalloc(common_info_len + 1);
	if (common_info == NULL) {
		DHD_ERROR(("failed to allocate buffer.\n"));
		return -ENOMEM;
	}

	/* Building the additional information like DHD, F/W version */
	memset(common_info, 0, common_info_len);
	snprintf(common_info, common_info_len, pre_strs, dhd_version, fw_version);
