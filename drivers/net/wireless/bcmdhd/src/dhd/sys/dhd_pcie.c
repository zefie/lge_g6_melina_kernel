 ...
 *
 * $Id: dhd_pcie.c 678589 2017-01-10 07:02:24Z $
 */
...
#endif /* CUSTOMER_HW4_DEBUG || CUSTOMER_HW10 */
		}
 ...
 * contexts
 * to gracefully exit. All the bus usage contexts before marking busstate as
 * busy, will check for
...
#ifndef CONFIG_MELINA_QUIET_DHD
		DHD_INFO(("%s: send H2D_HOST_D3_INFORM to dongle\n", __FUNCTION__));
#endif
		bus->d3_inform_cnt++;
	}
	if (h2d_mb_data == H2D_HOST_D0_INFORM_IN_USE) {
#ifndef CONFIG_MELINA_QUIET_DHD
		DHD_INFO(("%s: send H2D_HOST_D0_INFORM_IN_USE to dongle\n", __FUNCTION__));
#endif
		bus->d0_inform_in_use_cnt++;
	}
	if (h2d_mb_data == H2D_HOST_D0_INFORM) {
#ifndef CONFIG_MELINA_QUIET_DHD
		DHD_INFO(("%s: send H2D_HOST_D0_INFORM to dongle\n", __FUNCTION__));
#endif
		bus->d0_inform_cnt++;
	}
}
...
dhdpcie_handle_mb_data(dhd_bus_t *bus)
{
	...
	if (!d2h_mb_data) {
#ifndef CONFIG_MELINA_QUIET_DHD
		DHD_INFO(("%s: Invalid D2H_MB_DATA: 0x%08x\n",
