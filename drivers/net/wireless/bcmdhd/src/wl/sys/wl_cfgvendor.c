...
wl_cfgvendor_rtt_evt(void *ctx, void *rtt_data)
{
	...
	/* Alloc the SKB for vendor_event */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, tot_len, GOOGLE_RTT_COMPLETE_EVENT, kflags);
#else
	...
	const dhd_dbg_ring_status_t ring_status)
{
	...
	/* Alloc the SKB for vendor_event */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, len + 100, GOOGLE_DEBUG_RING_EVENT, kflags);
#else
	skb = cfg80211_vendor_event_alloc(wiphy, len + 100, GOOGLE_DEBUG_RING_EVENT, kflags);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,0) */
	if (!skb) {
		WL_ERR(("skb alloc failed"));
		return;
	...
	const uint32 len, const uint32 fw_len)
{
	...
	/* Alloc the SKB for vendor_event */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, len + 100, GOOGLE_FW_DUMP_EVENT, kflags);
#else
	skb = cfg80211_vendor_event_alloc(wiphy, len + 100, GOOGLE_FW_DUMP_EVENT, kflags);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,0) */
	if (!skb) {
		WL_ERR(("skb alloc failed"));
		return;
