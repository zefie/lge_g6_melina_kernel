...
void hdd_wlan_exit(hdd_context_t *pHddCtx)
{
   ...
   /* LGE_CHANGE, WIFI-5278, case#03650571 */
   /* Set comparam to VOS_STA_MODE by default since it won't clear
    * this flag for stop sap and static driver mode.
    */
...
int hdd_wlan_startup(struct device *dev, v_VOID_t *hif_sc)
{
   ...
   if (1)  /* LGE_CHANGE, WIFI-5278, case#03650571, remove the condition (VOS_STA_SAP_MODE != hdd_get_conparam()) */
   {
      /* Action frame registered in one adapter which will
       * applicable to all interfaces
       */
