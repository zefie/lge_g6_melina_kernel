...
int lgecrypto_sha1_update(struct shash_desc *desc, const u8 *data,
            unsigned int len)
{
    struct sha1_state *sctx = lgecryptow_shash_desc_ctx(desc);
    ...
    if ((partial + len) >= SHA1_BLOCK_SIZE) {
        ...
        do {
            lgecryptow_sha_transform(sctx->state, src, temp);
            done += SHA1_BLOCK_SIZE;
            src = data + done;
...
EXPORT_SYMBOL(lgecrypto_sha1_update);
...
static int sha1_final(struct shash_desc *desc, u8 *out)
{
    ...
    lgecrypto_sha1_update(desc, padding, padlen);

    /* Append length */
    lgecrypto_sha1_update(desc, (const u8 *)&bits, sizeof(bits));
...
static struct shash_alg alg = {
    ...
    .base        =    {
        .cra_name    =    "lge-sha1",
        .cra_driver_name=    "lge-sha1-generic",
