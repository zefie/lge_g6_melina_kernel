 ...
 *
 */
unsigned long lgecrypto_des_ekey(u32 *pe, const u8 *k)
{
...
EXPORT_SYMBOL_GPL(lgecrypto_des_ekey);
...
static int des_setkey(struct crypto_tfm *tfm, const u8 *key,
              unsigned int keylen)
{
    struct des_ctx *dctx = lgecryptow_crypto_tfm_ctx(tfm);
    u32 *flags = &tfm->crt_flags;
    u32 tmp[DES_EXPKEY_WORDS];
    int ret;

    /* Expand to tmp */
    ret = lgecrypto_des_ekey(tmp, key);
               ...
               unsigned int keylen)
{
    ...
    struct des3_ede_ctx *dctx = lgecryptow_crypto_tfm_ctx(tfm);
    ...
    lgecrypto_des_ekey(expkey, key); expkey += DES_EXPKEY_WORDS; key += DES_KEY_SIZE;
    dkey(expkey, key); expkey += DES_EXPKEY_WORDS; key += DES_KEY_SIZE;
    lgecrypto_des_ekey(expkey, key);

    return 0;
}

static int _des_init(struct crypto_tfm *tfm)
{
...
static struct crypto_alg des_alg = {
    .cra_name        =    "lge-des3_ede",
    .cra_driver_name    =   "lge-des-generic",
...
{
    return lgecryptow_crypto_register_alg(&des_alg);
}

void __exit des_generic_mod_fini(void)
{
    lgecryptow_crypto_unregister_alg(&des_alg);
}
