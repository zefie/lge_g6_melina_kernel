...
#define AFE_PORT_ID_TFA9872_RX AFE_PORT_ID_TERTIARY_MI2S_RX
#define AFE_PORT_ID_TFA9872_TX AFE_PORT_ID_TERTIARY_MI2S_TX
...
static int32_t afe_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			} else if(payload[0] == AFE_EVENT_TFADSP_RX_MODULE_STARTED) {
				pr_info("%s: AFE_EVENT_TFADSP_RX_MODULE_STARTED\n", __func__);
			} else if(payload[0] == AFE_EVENT_TFADSP_TX_MODULE_STARTED) {
				pr_info("%s: AFE_EVENT_TFADSP_TX_MODULE_STARTED\n", __func__);
			} else {
	...
	list_for_each_safe(ptr, next,
		...
		if (afe_top->path == path) {
			...
			if (afe_top->topology == 0x1000B900) {	  // check if afe id is for TFA Rx
				...
				if (path == TX_DEVICE && (port_id == AFE_PORT_ID_SLIMBUS_MULTI_CHAN_3_TX || port_id == AFE_PORT_ID_PRIMARY_PCM_TX)) {
					...
					pr_info("%s: changed top_id:%x for FM Radio(WCN3990) loopback Tx, acdb_id:%d afe_port_id:0x%x \n",
						__func__, afe_top->topology,
						afe_top->acdb_id, q6audio_get_port_id(port_id));
				} else if (path == TX_DEVICE && port_id != AFE_PORT_ID_TERTIARY_MI2S_RX) {
					//afe_top->topology = 0x000112F9; // forced reset topo id AFE_TX_TOPOLOGY(0x000112F9)
	...
	int dev, int buf_size, char *buf, int msg_type, int num_msgs)
{
	...
	afe_cal.hdr.token = index;
	afe_cal.hdr.opcode = AFE_PORT_CMD_SET_PARAM_V2;
	afe_cal.set_param.port_id = tfa_port_id;
