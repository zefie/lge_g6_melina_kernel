...
struct tfa_ls_compensation_filter {
  ...
  uint8_t ls_comp_on;  //      Loudspeaker compensation on/off; when 'off', the DSP code doesn't apply the bwExt => bw_ext_on GUI flag should be gray to avoid confusion
...
struct tfa_integrator_filter {
  ...
  float  cut_off_freq;        /**< cut off frequency in Hertz; range: [100.0 4000.0] */
  float  sampling_freq;      /**< sampling frequency in Hertz */
...
struct tfa_equalizer {
  struct tfa_filter filter[TFA98XX_MAX_EQ];// note: API index counts from 1..10
};
...
struct tfa_volume_step2_file {
	...
	uint8_t vsteps;  	// can also be calulated from size+type
	uint8_t samplerate; // ==enum samplerates, assure 8 bits
	struct tfa_volume_step2 vstep[]; 	//start of variable length contents:N times volsteps
};
